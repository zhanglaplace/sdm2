...
bool LinearRegressor::learn(cv::Mat &data, cv::Mat &labels, bool isPCA)
{
    ...
    if(this->isPCA){
        ...
        std::cout << "feature vector length: " <<FeaturePCA.eigenvectors.size() << std::endl;
        std::cout << "feature value length:   " <<FeaturePCA.eigenvalues.size() << std::endl;
        ...
        for(int i=0; i<FeaturePCA.eigenvalues.rows; i++){
            ...
            if(lamda/eigensum > 0.97){
                index = i;
                std::cout << "feature size encoding:" << i << " " << std::endl;
                break;
            }
        }
...
void ldmarkmodel::loadFaceDetModelFile(std::string filePath){
    ...
    if(face_cascade.empty()){
        std::cout << "faceDetect model load failed." << std::endl;
    }
}
...
void ldmarkmodel::train(std::vector<ImageLabel> &mImageLabels){
    ...
    std::cout << "input train size." << std::endl;
    std::cin >> samplesNum;
    if(samplesNum < 20)
        samplesNum = 20;
    else if(samplesNum > mImageLabels.size())
        samplesNum = mImageLabels.size();
    std::cout << "total " << samplesNum << "training samples.\n" << std::endl;

    std::cout << "normalized landmark error ?\n[Y/N?]" << std::endl;
    std::string t;
    std::cin >> t;
    if(t=="Y" || t=="y")
        isNormal = true;
...
void ldmarkmodel::printmodel(){
    if(isNormal)
        ...
        std::cout << "dont normalized" << std::endl;
    std::cout << "total" << LinearRegressors.size() << "iters." << std::endl;
    for(int i=0; i<LandmarkIndexs.size(); i++){
        std::cout <<"current"<<i<<"regression: "<<LandmarkIndexs.at(i).size()<<" pts  ";
